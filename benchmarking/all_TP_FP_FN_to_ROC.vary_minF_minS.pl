...
main: {
    ...
    print join("\t", "prog", "min_J", "min_S", "min_sum_frags", "TP", "FP", "FN", "TPR", "PPV", "F1") . "\n";

    for (my $min_J = 0; $min_J <= 5; $min_J++) {

        for (my $min_S = 0; $min_S <= 5; $min_S++) {
            
            
            foreach my $prog (keys %data) {
                my $progdata_href = $data{$prog};
                
                &make_ROC($prog, $progdata_href, $min_J, $min_S);
            }
        }
    }
    exit(0);
...
####
sub make_ROC {
    my ($prog_name, $progdata_href, $min_J, $min_S) = @_;
    ...
    @uniq_vals = grep { $_ >= $min_J + $min_S } @uniq_vals;

    @TP_fusions = grep { $_->{J} >= $min_J && $_->{S} >= $min_S} @TP_fusions;
    @FP_fusions = grep { $_->{J} >= $min_J && $_->{S} >= $min_S} @FP_fusions;
    
    for (my $i=0; $i < $#uniq_vals; $i++) {
        
        my $min_val = $uniq_vals[$i];
        
        @TP_fusions = grep { $_->{sum} >= $min_val  } @TP_fusions;
        
        @FP_fusions = grep { $_->{sum} >= $min_val } @FP_fusions;
        ...
        print join("\t", $prog_name, $min_J, $min_S, $min_val, $num_TP, $num_FP, $num_FN, $TPR, $PPV, $F1) . "\n";
    }
...
sub parse_file {
    ...
    while (<$fh>) {
        ...
        push (@{$data{$progname}->{$pred_type}}, { sum => $val,
                                                   J => $J,
                                                   S => $S } );
    }
    close $fh;
