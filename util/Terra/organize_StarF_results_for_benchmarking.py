...
def main():
    ...
    with open(local_files_list) as fh:
        for filename in fh:
            ...
            sample_name, count = re.subn(".STAR-Fusion.tsv.gz", "", sample_name)
            if count != 1:
                raise RuntimeError("didn't find .STAR-Fusion.tsv.gz in sample_name: {}".format(sample_name))
            ...
            outputfile = os.path.join(outdir, "star-fusion.fusion_predictions.abridged.tsv")
